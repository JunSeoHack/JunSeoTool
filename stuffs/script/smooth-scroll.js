...
}
{
   ...
   */ (function (root, factory) {
      ...
      var setHistory = function (options) {
          ...
          hash = hash ? hash : "";
          history.replaceState({ smoothScroll: JSON.stringify(options), anchor: hash ? hash : window.pageYOffset }, document.title, hash ? hash : window.location.href);
      };
      var updateURL = function (anchor, isNum, options) {
          if (isNum) return;
          if (!history.pushState || !options.updateURL) return;
          history.pushState({ smoothScroll: JSON.stringify(options), anchor: anchor.id }, document.title, anchor === document.documentElement ? "#top" : "#" + anchor.id);
      };
      ...
      var emitEvent = function (type, options, anchor, toggle) {
          ...
          var event = new CustomEvent(type, { bubbles: true, detail: { anchor: anchor, toggle: toggle } });
